...
- [x] 0.2 写清用户前端的需求、设计和页面边界
  - 状态：DONE
  - 这一步到底做什么：把首页、家庭、对话、记忆、设置五大入口和“完整但易用”的配置策略写清楚。
...
- [x] 1.3 建主导航、页面布局和家庭上下文切换
  - 状态：DONE
  - 这一步到底做什么：把首页、家庭、对话、记忆、设置五大入口和当前家庭切换先搭起来。
...
- [ ] 2.0 先把真实数据接入顺序钉住，再按顺序替换模拟数据
  - 状态：IN_PROGRESS
  - 当前结论：先做“全局数据底座和家庭上下文”，再做“家庭页主数据读链路”，再做“首页聚合”，最后做“设置里的设备 / HA / AI 持久化”。`AI配置 / 对话 / 记忆` 留在阶段 3，不混进这轮。
...
## 阶段 3：把 AI配置、对话和记忆做成产品中心

- [ ] 3.1 做“对话”页的多会话与连续对话能力
  - 状态：BLOCKED
  - 当前进度：原助手页已经接上真实 `family-qa/suggestions` 和 `family-qa/query`，也补了基于本地 `localStorage` 的最近会话恢复，说明用户端交互壳子已经大致成立。
  - 当前阻塞：产品方向已经改成 `AI配置 + 多Agent + 对话`；在后端和 `admin-web` 还没把多 Agent 配置、服务端会话持久化和 Agent 路由语义立住之前，继续改 `user-web` 只会制造补丁，所以这一步当前按策略暂停。
  - 这一步到底做什么：把原“助手页”升级成真正能长期使用的“对话中心”，而不是一次性问答框。
  - 做完你能看到什么：用户能在“对话”页管理多个会话，并在同一会话中持续追问，还能切换不同 Agent。
  - 先依赖什么：2.4
  - 开始前先看：
    - `requirements.md` 需求 6、9
    - `design.md` §2.3.3「对话会话流」
    - `design.md` §3.5「对话页面」
    - `design.md` §4.7、§4.8
  - 主要改哪里：
    - `apps/user-web/src/pages/assistant/`
    - `apps/user-web/src/components/assistant/`
  - 这一步先不做什么：在后端和 `admin-web` 前置条件没补齐前，先不继续推进 `user-web` 的对话页细化。
  - 怎么算完成：
    1. 多会话、新建会话、切换会话可用
    2. 连续对话上下文稳定
    3. 可基于多 Agent 语义切换对话上下文
  - 怎么验证：
    - `cd apps/user-web && npm.cmd run build`
    - 人工走查多轮对话和失败降级
...
- [ ] 3.3 打通对话到提醒、场景和记忆的快捷动作
  - 状态：BLOCKED
  - 当前进度：原助手页里的“转为提醒”“写入记忆”已经接到现有 `reminders` 和 `memories/cards/manual` 接口，也补了跳转到家庭页、设置页、记忆页的快捷入口。
  - 当前阻塞：当前产品方向已经从单助手转为 `AI配置 + 多Agent + 对话`。在后端和 `admin-web` 还没先把多 Agent 的动作边界、会话归属和权限语义理清前，不适合继续在 `user-web` 上扩动作壳子。
  - 这一步到底做什么：让“对话”回答不是停在一段文字，而是能继续转成家庭服务动作。
  - 做完你能看到什么：用户可以把回答转成提醒、写入记忆或跳转相关家庭页面，且动作边界对不同 Agent 是清楚的。
  - 先依赖什么：3.1、3.2
  - 开始前先看：
  ...
  - 主要改哪里：
    ...
    - 相关页面跳转与弹层
  - 这一步先不做什么：在多 Agent 后端语义没站稳前，先不继续扩 `user-web` 快捷动作。
  - 怎么算完成：
    1. 回答后能触发快捷动作
    2. 快捷动作有明确确认和结果反馈
  - 怎么验证：
...
- [ ] 3.4 检查 AI配置、对话和记忆是不是已经形成产品中心
  - 状态：TODO
  - 这一步到底做什么：确认用户端的核心价值已经立住，而不是只有几个静态页面。
  - 做完你能看到什么：首页、AI配置、对话、记忆已经形成一套能演示的产品主线。
  - 先依赖什么：3.1、3.2、3.3
  - 开始前先看：
