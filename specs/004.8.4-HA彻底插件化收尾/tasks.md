...
- [x] 1.2 产出当前 HA 残留基线和白名单初稿
  ...
  - 怎么验证：
    - 固定 grep 走查
    - 人工复核分类是否完整
  - 当前执行说明：已在 `docs/20260319-HA彻底插件化边界验收口径.md` 中补齐带编号的残留基线，覆盖核心越界、插件越界、前端越界和 spec 漂移；同时给出第一版白名单初稿，并明确哪些残留目前不准进白名单。
  - 对应需求：`requirements.md` 需求 1、需求 5、需求 6、需求 7
  - 对应设计：`design.md` §3.2、§3.3.1、§4.1

### 阶段检查

- [x] 1.3 阶段检查：确认这次不是再打一场糊涂仗
  - 状态：DONE
  ...
  - 怎么验证：
    - 人工走查
  - 当前执行说明：`004.8.4` 已建立主口径，边界验收文档已补齐编号化残留基线和白名单初稿；同时 `005.4` 的 2.1、2.2、4.2、4.4 已回写为“链路已打通但边界未收干净”的真实状态，第一阶段可以结束。
  - 对应需求：`requirements.md` 需求 1、需求 6、需求 7
...
- [x] 2.1 清理核心模块中的 HA 专名逻辑和平台特判
  ...
  - 怎么验证：
    - 核心目录 grep
    - 配置、同步、控制、状态读取主链回归
  - 当前执行说明：`device/service.py`、`device_control/router.py`、`device_integration/service.py`、`integration/service.py`、`context/service.py`、`conversation/orchestrator.py` 和 `plugin/dashboard_service.py` 的顶层 HA 专名逻辑已经清理；核心 grep 只剩 `context/schemas.py` 的兼容 alias / 兼容输出字段，不再承载 HA 业务分支。
...
- [x] 2.2 补齐宿主缺失的正式抽象接口
  ...
  - 怎么验证：
    - contract 单元测试
    - 相关主链集成测试
  - 当前执行说明：宿主已补出 `IntegrationInstanceRuntimeConfig`、`get_integration_instance_runtime_config(...)`、`runtime_config` payload 注入和 `live_state_snapshot` 同步 contract；HA 插件运行、同步和实时态读取都通过正式接口完成。
  - 对应需求：`requirements.md` 需求 2、需求 3
  - 对应设计：`design.md` §3.1、§3.3.1、§6.1、§6.2

### 阶段检查

- [x] 2.3 阶段检查：确认核心已经不再偷偷当 HA 宿主业务层
  - 状态：DONE
  ...
  - 怎么验证：
    - 核心 grep
    - 关键主链回放
  - 当前执行说明：核心目录复跑 grep 后，只剩 `context/schemas.py` 的受控兼容桥接；关键设备控制、设备同步和实时态主链已通过回归，未再发现宿主偷偷承担 HA 专用业务的入口。
  - 对应需求：`requirements.md` 需求 2、需求 5、需求 6
...
- [x] 3.1 清理 HA 插件对核心仓储、密钥和数据库的直连
  ...
  - 当前执行说明：`homeassistant_device_action/runtime.py` 已删除，`adapter.py` 与 `integration.py` 已改成直接消费宿主注入的 `runtime_config`；插件目录 grep 已清零 `repository`、`config_crypto` 和 `database_url` 越界依赖。
  - 对应需求：`requirements.md` 需求 3、需求 5、需求 6
  - 对应设计：`design.md` §3.3.1、§5.3、§6.2

- [x] 3.2 收口 HA 配置、同步和状态读取 contract
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - HA 点击同步回归
    - 设备状态加载回归
  - 当前执行说明：保存配置、读取配置、执行同步和实时态读取都已统一到实例级 `runtime_config` 语义，原始“点同步提示配置未完成”的双口径已经收掉；宿主仍保留对历史 `scope_type="plugin"` 脏数据的受控兼容读取，这一项已登记到边界白名单。
  - 对应需求：`requirements.md` 需求 3、需求 5、需求 6
  - 对应设计：`design.md` §2.2、§3.2、§3.3.1、§6.2

### 阶段检查

- [x] 3.3 阶段检查：确认 HA 插件已经失去特权通道
  - 状态：DONE
  ...
  - 怎么验证：
    - 插件目录 grep
    - 关键配置与同步回归
  - 当前执行说明：插件目录复跑 grep 后，越界关键词只剩平台实现本体；关键配置、同步和实时态链路已通过后端回归，HA 插件不再保留偷读宿主内部实现的后门。
  - 对应需求：`requirements.md` 需求 3、需求 5、需求 6
...
- [x] 4.1 清理前端顶层 HA 特例文案、类型和流程
  ...
  - 怎么验证：
    - 前端目录 grep
    - 统一实例与资源链路回归
  - 当前执行说明：`settings/index.tsx`、`settings/integrations/index.tsx`、`pageMessages.*` 和 `packages/user-core/src/domain/types.ts` 已清掉前端顶层 `home_assistant_status`、Home Assistant 文案和旧 HA 专用 API / 类型；`packages/user-core/src/api/create-api-client.ts` 只保留受控兼容归一化，用来把旧响应字段桥接成 `platform_health_status`；`npm run typecheck:user-app` 已通过。
  - 对应需求：`requirements.md` 需求 4、需求 6
  - 对应设计：`design.md` §2.2、§3.3.1

- [x] 4.2 回写 `005.1`、`005.4`、`005.6` 和相关文档的真实进度
  - 状态：DONE
  ...
  - 怎么验证：
    - 文档人工走查
    - 抽样对照代码残留
  - 当前执行说明：`004.8.4` 边界验收口径和 `005.4/tasks.md` 已按真实实现回写，已关闭的项直接标明 `DONE / closed`，保留中的后端 schema 兼容桥接和前端 API client 兼容归一化也都已写明白名单和删除条件。
  - 对应需求：`requirements.md` 需求 1、需求 7
  - 对应设计：`design.md` §2.1、§4.1、§4.2

### 阶段检查

- [x] 4.3 阶段检查：确认产品层和文档层不再帮旧逻辑打掩护
  - 状态：DONE
  ...
  - 怎么验证：
    - 前端 grep
    - 文档走查
  - 当前执行说明：前端顶层 grep 已清零 HA 专名，文档里关于完成度的表述也已对齐真实边界；产品层和 spec 层不再继续帮旧逻辑打掩护。
...
- [x] 5.1 产出最终 grep、白名单和剩余残留证据包
  ...
  - 怎么验证：
    - 复跑 grep
    - 人工核对白名单
  - 当前执行说明：最终 grep 结果已经收口到“核心只剩 `context/schemas.py` 兼容桥接，前端共享 API client 只剩 `create-api-client.ts` 兼容归一化，插件与前端顶层零残留”；边界验收文档已回写 `closed / whitelisted` 状态，并补充删除条件。
...
- [x] 5.2 完成最终回归和迁移校验
  ...
  - 怎么验证：
    - 执行测试集
    - 人工回放关键场景
  - 当前执行说明：`py -3 -m compileall ...` 已通过；`tests.test_device_integration_phase3.DeviceIntegrationPhase3Tests`、`tests.test_device_control_phase1.DeviceControlPhase1Tests`、`tests.test_device_control_phase2.DeviceControlPhase2Tests` 共 22 个关键后端回归已通过；`npm run typecheck:user-app` 已通过；上下文字段兼容输出也已人工校验。
  - 对应需求：`requirements.md` 需求 5、需求 6
  - 对应设计：`design.md` §7.1、§7.2、§7.3

### 最终检查

- [x] 5.3 最终检查点：只有证据齐全，才允许说“HA 彻底插件化完成”
  - 状态：DONE
  ...
  - 怎么验证：
    - 按本 spec 验收清单逐项核对
  - 当前执行说明：成功定义已经逐项对上实现、grep、白名单和测试证据；目前保留项只有 `context/schemas.py` 的后端兼容桥接和 `packages/user-core/src/api/create-api-client.ts` 的前端兼容归一化，并且都已被显式登记和说明删除条件，因此可以宣布 `004.8.4` 完成。
