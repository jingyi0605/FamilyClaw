...
- 让第三方开发者能按统一规则开发插件，让市场能按统一规则识别插件，并且别再误解成“第三方插件会被 HTTP 直接同步执行”
...
- [x] 1.3 开发规范检查点
  - 状态：已完成（2026-03-13，已补强插件对接说明；后续又补了后台任务和 runner 运行语义）
  - 这一步到底做什么：确认第三方开发者已经能靠规范把插件做出来，而不是还得翻项目源码猜。
  - 做完你能看到什么：后面可以开始写注册表规则和提交流程，而且不会再把第三方插件运行方式讲成旧的同步执行模型。
  - 先依赖什么：1.1、1.2
  - 开始前先看：
    - `requirements.md`
    - `design.md`
...
- [ ] 3.1 补开发样例和注册表示例
  - 状态：进行中（2026-03-13，已补开发环境、本地调试、测试与项目内运行验证文档；已补同容器子进程 runner 最小设计；后续又补了后台任务、worker、查询/通知/响应的文档语义）
...
- 状态：第一轮最小改造已完成（2026-03-13，已完成执行接口拆分、builtin 执行器保留、third-party runner 执行器骨架、`execute_plugin()` 分发入口、builtin 路径最小回归测试；已补 third-party `memory-ingestor` 分发、runner 失败场景测试、内部挂载配置表与最小管理接口）
