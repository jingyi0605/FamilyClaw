...
| `capabilities` | object | 否 | 受控上下文读取和地区 provider 运行声明 | 本轮已正式可用 |
...
| `capabilities` | 声明插件要读取哪些受控系统上下文，也声明是否要把自己作为地区 provider 挂进系统 | 避免后面每种扩展都另发明一套字段 |
...
- `reserved=true`：只是先占位，不运行
- `reserved=false`：运行时真的会把这个插件注册成地区 provider

别写假文档：

- 这轮正式可用的是“插件读取地区上下文”
- 这轮正式可用的还有“第三方地区 provider 通过 runner 子进程加载运行”
- 这轮依然没做的是“远程下载、市场分发、全局租户共享 provider”
