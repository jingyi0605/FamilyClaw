...
2. 运维或维护者把插件代码放到第三方插件目录，并准备插件自己的 venv。
3. 主服务调用 runner，而不是直接 import 第三方插件模块。
4. runner 在子进程里加载 `manifest.entrypoints` 指向的函数，传入 `payload`。
5. runner 把插件返回值转成 JSON 结果，回给主服务。
6. 主服务继续复用现有原始记录保存、记忆写入、权限校验、审计记录这些底座。
