...
## 阶段 4：把已收录插件的新版本自动发现补上

- [x] 4.1 实现已收录插件的轻量定时版本扫描
  - 状态：DONE
  - 完成日期：2026-03-20
  - 这一步到底做什么：给市场仓库补一条定时任务，扫描所有已收录插件的 `source_repo`，只在发现新增 tag 时补写版本信息。
  - 做完你能看到什么：插件第一次收录后，后面发新 tag 不用每次重新提 Issue，市场仓库会自动开版本更新 PR。
  - 先依赖什么：3.4
  - 开始前先看：
    - `requirements.md` 需求 8
    - `design.md` §2.3.6「已收录插件版本定时同步流程」
  - 主要改哪里：
    - 市场仓库 `scripts/sync_marketplace_versions.py`
    - 市场仓库 `.github/workflows/marketplace-version-sync.yml`
    - 市场仓库 `docs/contributing/`
    - 主仓库插件提交文档和当前 Spec
  - 这一步先不做什么：不做“上游删 tag 就自动删历史版本”，不把版本同步和收录 Issue 状态机硬拧到一起。
  - 完成内容：
    - 新增轻量扫描脚本，只先看 release/tag 列表，只有发现新增 tag 才读取对应 `manifest.json`
    - 新增每 2 小时执行一次的 GitHub Actions workflow，并保留手工触发入口
    - 版本同步只会追加新版本或把同版本 branch 记录收口为 tag 记录，不会自动删历史版本
    - 开发者文档和市场仓库文档已同步说明“已收录后如何进入新版本”
  - 怎么算完成：
    1. 已收录插件发新 tag 后，定时任务能生成或更新版本同步 PR
    2. 没有版本变化时，不会制造空 PR
    3. 上游删 tag 时，不会自动删市场历史版本
  - 怎么验证：
    - 本地单测覆盖“新增版本”和“branch 收口为 tag”
    - 用真实市场仓库条目跑一次脚本，确认能发现新 tag
  - 对应需求：`requirements.md` 需求 8
  - 对应设计：`design.md` §2.3.6、§3.3.4、§6.4
