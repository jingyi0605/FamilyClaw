...
- 宿主通用 `speaker_adapter` manifest 契约、`speaker_schemas` DTO、`speaker_host_service` 桥接服务已经落地，第三方插件可以按正式入口接入 `text_turn`、动作执行和 runtime heartbeat。
- 宿主里原来针对 `open-xiaoai-speaker` 的 discovery、binding、integration 历史特判链路已经继续抽干，核心现在按能力、实例和设备绑定关系处理，不再把某个插件名写死在 speaker 主链里。
- 之前试做过的“宿主 callback 认证会话”方案已经撤销，这不是 `005.9` 的正式宿主能力；宿主当前只保留通用 `config_preview`、`runtime_state` 和 `preview_artifacts` 展示契约。
