...
- [x] 3.1 建立新应用的主导航壳、认证壳和页面路由
  - 状态：DONE
  - 当前结果：`user-app` 的启动分流和受保护路由已经补修完成并通过复核：`entry` / 受保护页在启动摘要加载失败时会保留真实错误，不再硬跳登录；失效的本地家庭 ID 也不会再把已登录用户错误送去 `setup`；`login`、`setup`、主导航壳和受保护页面分流现在都直接站在共享启动摘要之上。`login` 页也已经从临时调试壳替换为按 `user-web` 旧页布局、文案和主题切换原样迁移的正式登录页，并补了 RN 安全样式分流，保证 H5 对齐旧页同时不打坏 iOS / Android 构建。
...
- [x] 3.2 迁首页、家庭、设置三条高频链路
  - 状态：DONE
  - 当前结果：`home`、`family`、`settings` 三条高频链路已完成补修并通过复核：家庭切换后的加载请求已加防串写保护，旧家庭数据不会再回流覆盖新家庭页面；`family` 新建成员账号改为走正式家庭账号接口，不会再误用 bootstrap 完成接口把当前管理员会话踢掉；首页、家庭、设置的加载、保存、空态、错误态与刷新一致性现在都落在新主壳里。
...
- [x] 3.3 迁助手、记忆和其余核心业务链路
  - 状态：DONE
  - 当前结果：`assistant` 已补修实时发送状态、会话切换时的助手对齐、websocket 发送失败后的 HTTP 回退，以及实时断开后的同步反馈；`memories` 已迁入 `user-app`，共享层补齐记忆模型与 API，页面已接入列表、分类搜索、详情、纠错、标记失效、删除、修订历史与家庭切换刷新；`plugins` 也已完成主链迁移：共享层补齐插件注册表 / 挂载 / 状态 / 任务模型与 API，`apps/user-app/src/pages/plugins/index.tsx` 已接入列表加载、详情查看、挂载、启用 / 禁用、删除挂载、最近任务、空态 / 错误态与家庭切换后的刷新一致性；`setup` 现也已收口：`user-core` 已补齐 AI provider / route / butler bootstrap 共享模型、API 与 setup helper，`apps/user-app/src/pages/setup/index.tsx` 已接入创建家庭、首位成员、AI 配置最小主链和首位管家实时初始化，并覆盖状态反馈、错误态与家庭切换后的刷新一致性。原先卡住的剩余主链已经清掉，因此保持 `DONE`。
...
- [ ] 3.4 完成 H5 灰度切流与 user-web 平替验证
  - 状态：IN_PROGRESS
  - 当前结果：本轮已把当前阻塞 H5 平替的三块真实设置入口补齐：`settings/ai` 已落地正式长期设置页，覆盖 provider 管理、能力路由、首位管家补建和 Agent 配置中心；`settings/integrations` 已落地 Home Assistant 配置、设备同步、房间同步、语音终端发现 / 认领；`settings/channel-access` 已落地平台账号管理、状态探测、失败记录和成员绑定。`npm.cmd --prefix ./apps/user-app run typecheck`、`npm.cmd run build:user-app:h5`、`npm.cmd run build:user-app:ios`、`npm.cmd run build:user-app:android` 已通过。当前判断已经从“准备态”进入“可以执行真实切流的候选态”，但本轮明确不做真实灰度切流、回滚演练、发布入口改造和网关 / 部署接流，所以不能写成 `DONE`。H5 入口包体积 warning 继续记为明确性能风险，不单独推翻候选态结论。
  ...
  - 这一轮补的前置材料：
    ...
    - `docs/20260315-H5平替评估与3.4-3.5检查清单.md`
...
- [ ] 3.5 阶段检查：H5 是不是真的可以替 user-web 说话了
  - 状态：TODO
  - 当前结果：从功能对齐角度看，原先阻塞 H5 平替的 `settings/ai`、`settings/integrations`、`settings/channel-access` 已全部落地，H5 不再被这三块真实入口卡住；但 `3.5` 要看的不是“功能有没有页面”这么简单，而是是否已经拿到真实灰度切流、回滚演练、发布入口接流和风险接受证据。本轮没有执行这些动作，所以还不能宣称 H5 已完成整体平替，也不能把 `3.5` 写成完成。
