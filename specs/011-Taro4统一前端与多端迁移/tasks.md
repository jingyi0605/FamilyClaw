...
- [ ] 3.1 建立新应用的主导航壳、认证壳和页面路由
  ...
  - 当前结果：`user-app` 的初始化向导页已从“只立壳和路由”升级为“按 `user-web` 旧页正式迁移”，H5 端使用本地化实现保留原页面结构、步骤流、欢迎层、主题切换、地区选择、AI 供应商配置和管家引导对话；RN 端暂留安全占位，避免被网页表单和浏览器实时链路拖垮构建。
  - 先依赖什么：2.4
  - 开始前先看：
...
- [ ] 3.2 迁首页、家庭、设置三条高频链路
  ...
  - 当前结果：`apps/user-app/src/pages/home/` 已从“功能接入”升级到“旧页正式迁移”；`apps/user-app/src/pages/family/` 也已从“只立入口壳”升级到“按 `apps/user-web/src/pages/FamilyPage.tsx` 的旧页正式迁移”，H5 端保留原页面结构、房间/成员/关系 UI、文案、图谱、空态和错误态，RN 端暂留安全构建页避免旧网页样式直接拖垮多端构建；设置页仍继续补齐。
