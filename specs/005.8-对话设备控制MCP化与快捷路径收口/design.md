...
1. 对话入口识别这是设备控制请求。
2. `device_shortcut_service` 先按家庭、成员和 `normalized_text` 做精确命中。
3. 精确命中后，系统检查目标设备、目标实体和动作当前是否仍然有效；失效记录标记为 `stale`。
4. 精确未命中时，再对当前家庭下的 active shortcuts 做一次低风险语义匹配。
5. 语义匹配会先按设备类型归一化动作语义，再从用户输入里剥掉动作词，只比较目标短语与历史短语、设备名、实体名的重合度。
6. 只有分数达到阈值且第一名明显优于第二名时，语义匹配才算命中；如果多个候选太接近，直接返回未命中。
7. 命中后直接生成执行计划，进入现有统一执行链。
8. 执行成功后更新命中次数和最近使用时间；执行失败则视情况降级回工具决策。
...
3. 若已有同目标的快捷路径被精确命中或语义命中，则更新已有记录并累加命中数据；否则新增记录。
...
- 输出：命中的快捷路径或 `None`
- 校验：先做 `normalized_text` 精确命中，精确失败后再做低风险语义兜底；只返回 `status=active` 且目标设备、实体仍可用的记录
- 语义兜底：按设备类型归一化动作语义，提取目标短语，对候选 shortcut 的历史短语、设备名、实体名做重合度评分；分数不足或候选歧义时返回 `None`
...
- `shortcut_match_ambiguous`：语义兜底阶段出现多个接近候选，系统拒绝盲猜
...
- “打开客厅灯”首次成功后，再说“开启客厅灯”会复用同一条快捷路径
- “打开灯”在多房间歧义时返回追问
- “确认解锁入户门锁”继续走高风险保护
