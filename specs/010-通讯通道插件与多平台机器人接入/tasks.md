...
- [x] 4.2 在”通讯平台接入”页的对应平台账号下补成员绑定编辑区
  - 状态：DONE
  - 完成说明：已在平台账号详情区集成 `ChannelAccountBindingsPanel` 组件，支持绑定列表展示、新增/编辑绑定弹窗、成员下拉选择（从当前家庭成员 API 加载）、绑定停用/恢复操作。冲突提示通过表单错误信息展示。后端绑定接口已补齐，支持从平台账号视角管理绑定。
  - 测试说明（2026-03-14）：已补充后端单元测试 `tests/test_channel_binding.py`，覆盖以下场景：
    - 创建绑定（成功、外部用户 ID 冲突检测、成员归属校验、空白字符处理、可选字段处理）
    - 更新绑定（external_user_id、display_hint、binding_status、冲突检测、同账号校验）
    - 列出绑定（空列表、返回绑定列表）
