...
- [x] 1.1 定义家庭级插件覆盖状态模型
  ...
  - 按文件拆开看：
    ...
    - `apps/api-server/migrations/versions/`：新增 migration，创建覆盖状态表和唯一索引。
  ...
  - 怎么验证：
    ...
    - 核对 `apps/api-server/migrations/versions/20260315_0033_create_plugin_state_overrides.py`
...
- [x] 1.3 状态模型检查点
  ...
  - 怎么算完成：
    ...
    2. 现有默认行为未被破坏
  - 怎么验证：
    - 人工走查最终状态公式：`enabled = base_enabled AND coalesce(household_enabled, true)`
    - `python -m unittest tests.test_plugin_manifest`
...
- [x] 2.1 改插件管理页和详情页的状态来源
  ...
  - 怎么算完成：
    ...
    2. 刷新页面后状态保持一致
  - 怎么验证：
    - 代码走查 `apps/user-web/src/pages/SettingsPluginsPage.tsx`
    - 代码走查 `apps/user-web/src/components/PluginDetailDrawer.tsx`
  - 对应需求：`requirements.md` 需求 2、需求 4
  - 对应设计：`design.md` §2.3.1、§3.3.1、§3.3.2

- [x] 2.2 提供统一插件状态更新接口并接到前端
  - 状态：已完成（2026-03-15）
  ...
  - 怎么算完成：
    ...
    2. 失败时能返回明确错误并展示给用户
  - 怎么验证：
    - `python -m unittest tests.test_plugin_jobs`
    - 代码走查 `apps/api-server/app/api/v1/endpoints/ai_config.py`
  - 对应需求：`requirements.md` 需求 2、需求 4
...
- [x] 3.2 给通道、地区 provider、语言包链路接统一可用性判断
  - 状态：已完成（2026-03-15）
  ...
  - 怎么验证：
    - `python -m unittest tests.test_plugin_locales_api`
    - `python -m unittest tests.test_plugin_region_provider_runtime`
    - 人工走查 channel / scheduler / locale 入口已切到统一判断
...
- [x] 3.3 停用收口检查点
  ...
  - 怎么算完成：
    ...
    2. 前端和后端提示一致
  - 怎么验证：
    - 关键回归测试：`test_plugin_manifest`、`test_agent_plugin_bridge`、`test_plugin_runs`、`test_plugin_jobs`、`test_plugin_locales_api`、`test_plugin_region_provider_runtime`
    - 人工走查禁用后任务、Agent、channel、region provider、locale 都走统一最终状态
  - 对应需求：`requirements.md` 需求 3、需求 4
  - 对应设计：`design.md` §2.3.3、§6

## 阶段 4：补旧 Spec 和最终验收

- [x] 4.1 更新旧插件系统 Spec 的相关描述
  - 状态：已完成（2026-03-15）
...
- [ ] 4.2 最终检查点
  ...
  - 怎么算完成：
    ...
    3. 旧 Spec 已更新到位
  - 怎么验证：
    - 按主链路做一次完整人工验收（当前还没在真实页面上跑完整手工验收）
    - 核对关键测试结果（已完成）
