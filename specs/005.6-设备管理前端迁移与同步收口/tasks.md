...
- [x] 1.2 补齐 HA 接口与筛选字段分析
  - 状态：DONE
  - 这一步到底做什么：明确名称、房间、集成分类这三个筛选条件分别来自 HA 哪些接口和哪些字段。
...
- [x] 3.3 补部分同步的搜索与筛选
  - 状态：DONE
  - 这一步到底做什么：把候选设备弹层从“勾选列表”升级成可搜索、可按 房间和集成分类筛选的选择器。
...
- [x] 4.1 完成回归验证清单
  ...
  - 怎么算完成：
    ...
    3. 关键错误场景有反馈
  - 怎么验证：
    - 已执行 `npm.cmd --prefix apps/user-app run test:integration-sync`
    - 已执行 `npm.cmd --prefix apps/user-app run typecheck`
    - 未启动开发服务器，页面级人工回归暂以代码走查为准
    - 设置页已改为直接使用独立接口加载，避免依赖 `/integrations/page-view` 聚合接口和 `integration_discoveries` 表
