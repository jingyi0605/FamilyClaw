...
- [ ] 2.4 做对话页的 Agent 上下文切换能力
  - 状态：BLOCKED
  - 当前阻塞：当前已经明确这轮先不动 `user-web`，所以对话页的 Agent 切换能力暂不推进；后端显式 `agent_id` 选择骨架已完成，等用户端开始时直接接。
...
- [ ] 2.5 阶段检查：确认多Agent已经进运行时主链路
  - 状态：BLOCKED
  - 当前进度：后端主链路已经成立：`soul`、成员认知、长期记忆视角都已进入 `family_qa` 运行时，并且主管家兜底与显式 Agent 选择已经可用。当前唯一没完成的是 `2.4` 对话页切换，因为这轮明确不动 `user-web`。
  - 当前阻塞：`2.4` 依赖 `user-web`，而当前范围明确先不推进用户端，所以这个阶段检查不能假装 DONE。
