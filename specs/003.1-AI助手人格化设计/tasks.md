...
## 当前完成情况（截至 2026-03-11）

如果你现在只想知道“到底做到了哪”，先看这段，不要在整份任务里翻来翻去。

### 已完成

- [x] A1 先建多 Agent 数据表和迁移
- [x] A2 补 Agent 的 Schema、Repository 和服务骨架
- [x] A3 做 AI配置列表接口和单 Agent 详情接口
- [x] A4 做 Soul、成员认知、运行时策略更新接口
- [x] A5 补主管家兜底和多 Agent 运行时选择骨架
- [x] A6 在 `admin-web` 做 AI配置列表页
- [x] A7 在 `admin-web` 做单 Agent 配置页
- [x] A8 当前阶段检查：确认已经能支撑 AI配置页联调
- [x] 1.1 新增多 Agent 基础模型
...
- [x] 2.4 做对话页的 Agent 上下文切换能力
- [x] 2.5 阶段检查：确认多Agent已经进运行时主链路
- [ ] 3.x 用户端 `AI配置 + 对话`
- [ ] 4.x 治理、文档和最终验收

### 一句话判断

现在这份 Spec 的真实进度不是“刚开头”，而是：

- **阶段 1 的后端基座已经基本站住**
- **`admin-web` 的 AI配置已经能看、能改**
- **阶段 2 已经开始接运行时，但还没闭环**
- **`user-web` 已开始接 AI配置 + 对话，外观链路还没做**
- **正式产品里的新家庭初始化与完整 AI 配置入口，后续由 `003.2` / `003.3` 继续承接**
...
- 主要改哪里：
  ...
  - `apps/admin-web/`
- 实际完成：
  - 后端已经具备 Agent 模型、读写接口和主管家兜底骨架
  - `admin-web` 已有可查看、可编辑的 AI配置页
  - 当前问答链路已经能返回生效 Agent，并开始消费 `soul / 成员认知` 运行时上下文
...
- [x] 1.2 做 AI配置聚合读模型
  - 状态：DONE
  - 当前进度：A3/A4/A6/A7 已完成，当前已经具备稳定的 AI配置聚合读模型、详情读取和管理侧可编辑配置页。外观摘要仍未纳入，但不影响本阶段读模型闭环。
...
- [x] 1.3 阶段检查：确认已经不是单助手模式
  - 状态：DONE
  - 当前进度：A8 已完成。当前已经具备多 Agent 数据模型、AI配置读写接口、`admin-web` AI配置管理页，以及主管家兜底骨架，已经不是“一个助手 + 一堆配置项”的旧结构。
...
- [x] 2.1 接各 Agent 的 Soul 配置与运行时人格上下文
  - 状态：DONE
  - 当前进度：A4 已完成 `soul` 的写接口，A5 已完成主管家兜底与显式 Agent 选择，当前已经把 `soul` 真正拼进 `family_qa` 的运行时上下文和 `qa_generation` prompt。
...
- [x] 2.2 接各 Agent 的成员认知
  - 状态：DONE
  - 当前进度：A4 已完成成员认知写接口，当前已经把“对当前请求成员的称呼 / 沟通风格 / 注意事项”拼进 `family_qa` 运行时上下文。
...
- [x] 2.3 复用记忆中心承接多 Agent 长期记忆
  - 状态：DONE
  - 当前进度：当前已经把 Agent 的 `memory_scope` 真正接进 `family_qa` 的记忆摘要生成，也把“按 Agent 记忆视角筛出的长期记忆”喂给 `qa_generation`。同时，Agent 相关问答线索已经通过 `event_records` 写回统一记忆入口。这一版先复用现有记忆中心做过滤视角和事件写回，不新建独立记忆表。
...
- [x] 2.4 做对话页的 Agent 上下文切换能力
  - 状态：DONE
  - 当前进度：`user-web` 已接入多 Agent 对话切换：对话页默认落到主管家或默认入口 Agent，支持手动切换到其他已启用 Agent，并且切换时自动拆出新会话，避免不同 Agent 混用同一段上下文。
...
- [x] 2.5 阶段检查：确认多Agent已经进运行时主链路
  - 状态：DONE
  - 当前进度：后端主链路已经成立：`soul`、成员认知、长期记忆视角都已进入 `family_qa` 运行时；现在 `user-web` 对话页也已显式接入 Agent 切换、默认入口和问答请求里的 `agent_id`，多 Agent 已经真正进主链路。
  ...
  - 怎么算完成：
    1. 后端运行时上下文已按 Agent 维度组装
    2. `user-web` 对话页已能切换 Agent
    3. 降级路径已明确
  - 怎么验证：
    - 人工走查
    - 关键对话流程验证
...
- [x] 3.1 保留 AI配置 作为统一多Agent设置入口
  - 状态：DONE
  - 当前进度：`user-web` 设置侧已经保留 `AI配置` 入口，并改成多 Agent 列表 + 只读详情视图，能看到角色摘要、运行时状态、成员认知概览，并可从这里进入对话页。
...
- [x] 3.2 将原助手页面改造成对话页
  - 状态：DONE
  - 当前进度：导航文案和主入口已经改成“对话”；页面接上默认 Agent、手动切换、按 Agent 加载推荐问题、按 Agent 发起 `family_qa` 查询，并保留提醒/记忆落地动作。
