---
title: Docs Overview
docId: en-1.1
version: v0.1
status: draft
order: 110
outline: deep
---

# Docs Overview

## What this page solves

- It tells a first-time reader which pages to open within 30 seconds.
- It keeps the whole site locked to five top-level sections so new content does not sprawl into random buckets.

## Current top-level sections

- `Getting Started`: understand the product first, then decide whether to try it, install it, or develop it.
- `Installation`: Docker, source, local, NAS, Ubuntu, and Windows installation or deployment paths.
- `User Guide`: explains the real product pages such as dashboard, family, conversations, memory, settings, and plugins.
- `Developer Docs`: covers development environment, backend rules, plugin rules, and integration entry points.
- `Community`: official website, QQ group, WeChat group, and Discord entry points.

## Recommended reading paths

### I am touching FamilyClaw for the first time

1. Read [Product Overview](./product-overview.md) to understand what FamilyClaw actually does.
2. Then read [Quick Start](./quick-start.md) to choose the fastest path.
3. If you need it running, jump to [Installation](../installation-deployment/overview.md).
4. If you want to learn the pages, go to the [User Guide](../user-guide/first-login-and-setup.md).
5. If you need to develop or customize it, go to the [Developer Docs](../developer-docs/environment-setup.md).

### I need installation docs for a user

1. Open [Installation Overview](../installation-deployment/overview.md).
2. Choose Docker, source, NAS, Ubuntu, or Windows.
3. Send that single page instead of asking the reader to scan everything.

### I need to maintain the English docs

1. Start from the `/en/` homepage.
2. Follow the same section structure as the Chinese root.
3. Do not invent a second information architecture just for English.

## Completion standard

- A new reader can find the right entry point within 30 seconds.
- New content must land in one of the five top-level sections instead of creating new parallel structures.
