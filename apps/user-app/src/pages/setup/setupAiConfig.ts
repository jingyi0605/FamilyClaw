...
export function getCapabilityLabel(capability: string, locale = 'zh-CN') {
  const matched = AI_CAPABILITY_OPTIONS.find(item => item.value === capability);
  return matched ? getPageMessage(locale, matched.labelKey) : capability;
}
...
export function buildCreateProviderPayload(form: ReturnType<typeof buildProviderFormState>, adapter: AiProviderAdapter): AiProviderProfileCreatePayload {
  return {
...
export function readProviderFormValue(form: ReturnType<typeof buildProviderFormState>, fieldKey: string) {
...
export function assignProviderFormValue(form: ReturnType<typeof buildProviderFormState>, fieldKey: string, value: string) {
  ...
  return { ...form, dynamicFields: { ...form.dynamicFields, [fieldKey]: value } };
}
...
function buildDynamicExtraConfig(dynamicFields: Record<string, string>, adapter: AiProviderAdapter) {
  const result: Record<string, string | number | boolean> = {};
  for (const field of adapter.field_schema) {
    if (CORE_PROVIDER_FIELD_KEYS.has(field.key)) continue;
