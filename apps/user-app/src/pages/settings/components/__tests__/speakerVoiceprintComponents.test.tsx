...
test('向导会展示等待进度，并保证当前轮次和进度卡片一致', () => {
  const waitingMarkup = renderWithI18n(
    <VoiceprintEnrollmentWizard
      ...
      members={[createMemberSummary()]}
      deviceName="客厅小爱"
      enrollment={createEnrollment({ status: 'recording', sample_count: 2, sample_goal: 3 })}
      busy={false}
      onClose={() => {}}
      onCancelEnrollment={() => {}}
      onBack={() => {}}
  ...
  assert.match(waitingMarkup, /第 3 \/ 3 轮/);
  assert.match(waitingMarkup, /3 \/ 3 轮样本/);
