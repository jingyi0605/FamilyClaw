...
def build_async_home_assistant_client_for_household(db: Session, household_id: str) -> AsyncHomeAssistantClient:
    ...
    base_url = _normalize_optional_text(config.base_url) if config else None
    access_token = _normalize_optional_text(config.access_token) if config else None
    return AsyncHomeAssistantClient(base_url=base_url, token=access_token)
...
async def async_sync_home_assistant_devices(
    db: Session,
    *,
    household_id: str,
    external_device_ids: list[str] | None = None,
    client: AsyncHomeAssistantClient | None = None,
) -> SyncSummary:
    get_household_or_404(db, household_id)
    config = get_household_ha_config(db, household_id)
    client = client or build_async_home_assistant_client_for_household(db, household_id)
    device_registry, entity_registry, area_registry, states = await asyncio.gather(
    ...
    area_name_map = _build_area_name_map(area_registry)
    supported_entities_by_device, skipped_entities = _group_supported_entities_by_device(entity_registry)
    selected_device_ids = {device_id for device_id in (_normalize_optional_text(value) for value in (external_device_ids or [])) if device_id}
    ...
    for ha_device in device_registry:
        ...
        try:
            ...
            entity_id = entity_id_value
            domain = entity_id_value.split(".", 1)[0]
            device_type, controllable = SUPPORTED_DEVICE_TYPES[domain]
            state = state_map.get(entity_id)
            normalized_status = _normalize_ha_device_status(entity_entries, state_map)
            room_name = _resolve_room_name(
                ha_device=ha_device,
            ...
            device_name = _resolve_device_name(ha_device=ha_device, primary_entity=primary_entity, state=state)
            capabilities = {
                "ha_device_id": external_device_id,
                "entity_ids": [entry["entity_id"] for entry in entity_entries],
                "primary_entity_id": entity_id,
            ...
            with db.begin_nested():
                binding = _get_existing_binding(db, external_device_id=external_device_id, primary_entity_id=entity_id)
                if binding is None:
                    ...
                    updated_devices += 1
                if sync_rooms_enabled and room_name:
                    room, room_created = _get_or_create_room_from_name(db, household_id=household_id, room_name=room_name, room_cache=room_cache)
                    if room_created:
                        created_rooms += 1
                    if device.room_id != room.id:
                        device.room_id = room.id
            ...
            failures.append(SyncFailure(entity_id=entity_id, reason=str(exc)))

    if config is not None:
        config.last_device_sync_at = utc_now_iso()
        db.add(config)

    return SyncSummary(household_id=household_id, created_devices=created_devices, updated_devices=updated_devices, created_bindings=created_bindings, created_rooms=created_rooms, assigned_rooms=assigned_rooms, skipped_entities=skipped_entities, failed_entities=len(failures), devices=synced_devices, failures=failures)
...
async def async_list_home_assistant_device_candidates(
    db: Session,
    *,
    household_id: str,
    client: AsyncHomeAssistantClient | None = None,
    ...
    for ha_device in device_registry:
        ...
        domain = primary_entity_id.split(".", 1)[0]
        state = state_map.get(primary_entity_id)
        candidates.append(HaDeviceCandidate(external_device_id=external_device_id, primary_entity_id=primary_entity_id, name=_resolve_device_name(ha_device=ha_device, primary_entity=primary_entity, state=state), room_name=_resolve_room_name(ha_device=ha_device, primary_entity=primary_entity, area_name_map=area_name_map, state_map=state_map), device_type=SUPPORTED_DEVICE_TYPES[domain][0], entity_count=len(entity_entries), already_synced=external_device_id in existing_bindings))
    return sorted(candidates, key=lambda item: ((item.room_name or "未分配房间"), item.name, item.external_device_id))
...
async def async_sync_home_assistant_rooms(
    db: Session,
    *,
    household_id: str,
    room_names: list[str] | None = None,
    ...
    get_household_or_404(db, household_id)
    client = client or build_async_home_assistant_client_for_household(db, household_id)
    area_registry = await client.get_area_registry()
    room_cache = _load_room_cache(db, household_id)
    requested_names = {_normalize_room_key(name) for name in (room_names or []) if name.strip()}
    matched_entities = 0
    skipped_entities = 0
    ...
    for area in area_registry:
        ...
        if normalized_room_key in room_cache:
            raise ValueError(f"room already exists locally: {room_name}")
        matched_entities += 1
        _, created = _get_or_create_room_from_name(db, household_id=household_id, room_name=room_name, room_cache=room_cache)
        if created:
            created_rooms += 1
    return RoomSyncSummary(household_id=household_id, created_rooms=created_rooms, matched_entities=matched_entities, skipped_entities=skipped_entities, rooms=list(room_cache.values()))
...
async def async_list_home_assistant_room_candidates(
    db: Session,
    *,
    household_id: str,
    client: AsyncHomeAssistantClient | None = None,
    ...
    get_household_or_404(db, household_id)
    client = client or build_async_home_assistant_client_for_household(db, household_id)
    area_registry, device_registry, entity_registry = await asyncio.gather(
        client.get_area_registry(),
        client.get_device_registry(),
        client.get_entity_registry(),
    )
    ...
    for area in sorted(area_registry, key=lambda item: str(item.get("name", ""))):
        ...
        area_id = _normalize_optional_text(area.get("area_id"))
        exists_locally = room_key in room_cache
        candidates.append(HaRoomCandidate(name=room_name, entity_count=area_entity_counts.get(area_id or "", 0), exists_locally=exists_locally, can_sync=not exists_locally))
    return candidates
...
async def async_execute_home_assistant_device_action(
    db: Session,
    *,
    household_id: str,
    device: Device,
    ...
    if capabilities is not None and not isinstance(capabilities, dict):
        capabilities = None
    entity_id = _normalize_optional_text(binding.external_entity_id) or _normalize_optional_text((capabilities or {}).get("primary_entity_id"))
    if not entity_id:
        raise ValueError("home assistant entity id not found for device")
    service_domain, service_name = action_mapping[action]
    service_params = {"entity_id": entity_id, **_normalize_action_params(device_type=device.device_type, action=action, params=params)}
    client = client or build_async_home_assistant_client_for_household(db, household_id)
    response_payload = await client.call_service(domain=service_domain, service=service_name, data=service_params)
    return DeviceActionExecution(household_id=household_id, device_id=device.id, action=action, platform="home_assistant", service_domain=service_domain, service_name=service_name, entity_id=entity_id, params=service_params, response_payload=response_payload)
